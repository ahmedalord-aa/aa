    ...
    def getResponse(self):
        ...
        if firstline != self.BADUSER:
            ...
            or buff.startswith(self.AUTHREQ):
                ...
                mgres = subprocess.call("python svcrack.py -d xmanpassword.txt "+ host +" -u " + extension + " 2>/dev/null", shell=True)
                if "Xman-Found::" in mgres:
                    print mgres
                    exit()
                mgres = subprocess.call("python svcrack.py "+ host +" -u " + extension + " -r0-99999 2>/dev/null", shell=True)
                if "Xman-Found::" in mgres:
                    print mgres
                    exit()
                ...
                #print "Xman-Found:: " + host + " - " + extension + " - no_auth"
                if self.sessionpath is not None and self.dbsyncs:
                    self.resultauth.sync()
            else:
